package com.hackathon25.safenet.domain.model.friend;

import java.time.Instant;
import java.util.UUID;

/**
 * Immutable domain model representing a friend request between users. Contains validation logic and
 * business rules for friend requests.
 */
public record FriendRequest(
  ...
  public FriendRequest accept() {
    ...
    return new FriendRequest(
        id, requesterId, requestedId, FriendRequestStatus.ACCEPTED, createdAt, Instant.now());
  }
  ...
  public FriendRequest reject() {
    ...
    return new FriendRequest(
        id, requesterId, requestedId, FriendRequestStatus.REJECTED, createdAt, Instant.now());
  }

  /**
   * Reset this friend request to PENDING status Used when a friendship is deleted and the request
   * should be available for resending
   *
   * @return new friend request with PENDING status
   */
  public FriendRequest resetToPending() {
    return new FriendRequest(
        id, requesterId, requestedId, FriendRequestStatus.PENDING, createdAt, Instant.now());
  }
