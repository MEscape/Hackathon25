...
public class FriendshipJpaAdapter implements FriendshipRepositoryPort {
  ...
  public Friendship save(Friendship friendship) {
    log.debug(
        "Saving friendship: user1Id={}, user2Id={}", friendship.user1Id(), friendship.user2Id());
  ...
  public Optional<Friendship> findById(UUID id) {
    ...
    return jpaRepository.findById(id).map(friendshipMapper::toDomain);
  }
  ...
  public List<User> findMutualFriends(UUID user1Id, UUID user2Id) {
    ...
    return user1Friends.stream().filter(user2Friends::contains).toList();
  }
}
