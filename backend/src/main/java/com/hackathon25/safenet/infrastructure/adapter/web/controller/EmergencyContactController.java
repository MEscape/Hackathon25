 ...
 *
 * <p>This controller consolidates friend and user management operations to match the frontend API
 * structure. It provides endpoints for managing emergency contacts (friends) and discovering new
 * users.
 *
 * <h3>Supported Operations</h3>
 *
 * <ul>
 *   <li>Get Emergency Contacts - List all friends
 *   <li>Discover Users - Find users with request status
 *   <li>Send Friend Request - Request friendship with a user
 *   <li>Accept Friend Request - Accept incoming request
 *   <li>Decline Friend Request - Decline incoming request
 *   <li>Remove Emergency Contact - Unfriend a user
 * </ul>
 *
...
public class EmergencyContactController {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            ...
            content =
                @Content(schema = @Schema(implementation = EmergencyContactsResponseDto.class))),
  ...
  public ResponseEntity<EmergencyContactsResponseDto> getEmergencyContacts(
    ...
    List<EmergencyContactDto> contacts = friends.stream().map(EmergencyContactDto::from).toList();
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            ...
            content =
                @Content(schema = @Schema(implementation = DiscoverableUsersResponseDto.class))),
        @ApiResponse(responseCode = "401", description = "Unauthorized")
      })
  public ResponseEntity<DiscoverableUsersResponseDto> getDiscoverableUsers(
    ...
    List<DiscoverableUserDto> discoverableUserDtos =
        discoverableUsers.stream().map(this::mapToDto).toList();
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "409",
            description = "Request already exists or users are already friends")
      })
  public ResponseEntity<FriendRequestResponseDto> sendFriendRequest(
      @Parameter(description = "ID of the user to send friend request to") @PathVariable
          UUID userId,
      Authentication authentication) {
    ...
    return ResponseEntity.ok(FriendRequestResponseDto.success("Friend request sent successfully"));
  }
  ...
  public ResponseEntity<FriendRequestResponseDto> acceptFriendRequest(
      @Parameter(description = "ID of the user who sent the friend request") @PathVariable
          UUID userId,
      Authentication authentication) {
  ...
  public ResponseEntity<FriendRequestResponseDto> removeEmergencyContact(
      @Parameter(description = "ID of the friend to remove") @PathVariable UUID userId,
      Authentication authentication) {
    UUID currentUserId = UUID.fromString(authentication.getName());
  ...
  /**
   * Maps a domain DiscoverableUser to a DiscoverableUserDto. This mapping happens in the
   * infrastructure layer, converting domain objects to DTOs.
   *
