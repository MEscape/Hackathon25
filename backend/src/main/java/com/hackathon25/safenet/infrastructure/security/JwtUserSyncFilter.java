...
/**
 * Listens to authentication success events and syncs users from JWT token to local database. This
 * approach eliminates the need for webhooks by syncing users automatically on every successful
 * authentication. User data is extracted from the JWT token issued by Keycloak. Benefits: - 100%
 * reliable - no webhook delivery issues - Always syncs latest user data on login - No missing user
 * IDs - Simpler architecture
 */
@Slf4j
@Component
