 ...
 *
 * <p>This configuration adapts authentication strategy based on the active Spring profile:</p>
 * <ul>
 *   <li><strong>Development (dev):</strong> Uses JWT validation with JWK Set</li>
 *   <li><strong>Production (prod):</strong> Uses opaque token introspection</li>
 * </ul>
 *
 * <h3>Development Profile (JWT)</h3>
 * <p>Validates JWT tokens locally using Keycloak's public keys (JWK Set).
 * Faster and doesn't require a network call per request.</p>
 *
 * <h3>Production Profile (Opaque Token)</h3>
 * <p>Validates tokens via introspection endpoint. More secure as tokens can be
 * revoked immediately and validation is centralized.</p>
 */
@Configuration
...
public class ResourceServerSecurityConfig {
     ...
     *
     * Uses local JWT validation with JWK Set for faster token validation.
     * Suitable for development where token revocation is less critical.
     *
    ...
    public SecurityFilterChain jwtFilterChain(HttpSecurity http, JwtDecoder jwtDecoder) throws Exception {
        ...
        customDecoder.setJwtValidator(new DelegatingOAuth2TokenValidator<>(
     ...
     *
     * <p>Uses token introspection for validation. More secure as it allows
     * immediate token revocation and centralized validation.</p>
     *
     * @param http the HttpSecurity configuration object
