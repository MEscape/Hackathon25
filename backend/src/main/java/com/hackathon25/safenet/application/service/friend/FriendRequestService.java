 ...
 *
 * <p>Handles the complete lifecycle of friend requests including sending, accepting, rejecting, and
 * canceling requests. Ensures proper validation and state transitions according to business rules.
 *
...
public class FriendRequestService implements FriendRequestPort {
  ...
  private void validatePending(FriendRequest request) {
    if (request.status() != FriendRequest.FriendRequestStatus.PENDING) {
      throw new InvalidFriendRequestStatusException(
          request.status(), FriendRequest.FriendRequestStatus.PENDING);
    }
  }
  ...
  private FriendRequest getFriendRequestBetweenUsers(UUID requesterId, UUID userId) {
    return friendRequestRepository
        .findBetweenUsers(requesterId, userId)
        .orElseThrow(
            () ->
                new FriendRequestNotFoundException(
                    "Friend request not found between users: " + requesterId + " and " + userId));
  }
}
