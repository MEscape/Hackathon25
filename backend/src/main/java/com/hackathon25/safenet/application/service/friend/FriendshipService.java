 ...
 *
 * <p>This service handles all friendship-related operations including retrieving friends, managing
 * friendships, and providing friendship statistics.
 *
 * <h3>Key Features</h3>
 *
 * <ul>
 *   <li><strong>Friend Management:</strong> Comprehensive friendship operations and queries
 *   <li><strong>Validation:</strong> Proper validation for user existence and friendship status
 *   <li><strong>Statistics:</strong> Friend counts, mutual friends, and recent friendship data
 *   <li><strong>Performance:</strong> Optimized queries for large friend networks
 * </ul>
 *
 * <h3>Friendship Model</h3>
 *
 * <p>Friendships are bidirectional relationships stored as a single record with user1Id and
 * user2Id. The service handles the bidirectional nature transparently, allowing queries from either
 * user's perspective.
 *
...
public class FriendshipService implements FriendshipPort {
  ...
  public void removeFriendship(UUID userId1, UUID userId2) {
    log.info(
        "Removing friendship between users: {} and {}, removed by: {}", userId1, userId2, userId1);
