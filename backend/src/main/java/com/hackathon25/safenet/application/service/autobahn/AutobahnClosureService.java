...
public class AutobahnClosureService implements AutobahnClosurePort {
  ...
  public List<AutobahnClosureItem> getAutobahnClosureData(String autobahnId) {
    try {
      ...
      List<AutobahnClosureItemDto> closureDtos =
          autobahnClosureFeedPort.fetchClosureData(autobahnId);

      // Convert DTOs to domain objects
      List<AutobahnClosureItem> closures =
          closureDtos.stream().map(this::convertToItem).collect(Collectors.toList());
  ...
  private AutobahnClosureItem.Coordinate convertToCoordinate(
      AutobahnClosureItemDto.CoordinateDto dto) {
    return AutobahnClosureItem.Coordinate.builder().lat(dto.getLat()).lng(dto.getLng()).build();
  }
}
