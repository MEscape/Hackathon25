...
public class AutobahnClosureParser {
    ...
    private static AutobahnClosureItemDto parseClosureItem(JsonNode node) {
        try {
            AutobahnClosureItemDto.AutobahnClosureItemDtoBuilder builder = AutobahnClosureItemDto.builder();
            ...
            if (node.has("description") && node.get("description").isArray()) {
                List<String> descriptions = objectMapper.convertValue(node.get("description"), new TypeReference<>() {
                });
                builder.description(descriptions);
            }
            if (node.has("routeRecommendation") && node.get("routeRecommendation").isArray()) {
                List<String> recommendations = objectMapper.convertValue(node.get("routeRecommendation"), new TypeReference<>() {
                });
                builder.routeRecommendation(recommendations);
            }
            if (node.has("footer") && node.get("footer").isArray()) {
                List<String> footers = objectMapper.convertValue(node.get("footer"), new TypeReference<>() {
                });
                builder.footer(footers);
            }
            if (node.has("lorryParkingFeatureIcons") && node.get("lorryParkingFeatureIcons").isArray()) {
                List<String> icons = objectMapper.convertValue(node.get("lorryParkingFeatureIcons"), new TypeReference<>() {
                });
            ...
            if (node.has("coordinate")) {
                ...
                AutobahnClosureItemDto.CoordinateDto coordinate = AutobahnClosureItemDto.CoordinateDto.builder()
