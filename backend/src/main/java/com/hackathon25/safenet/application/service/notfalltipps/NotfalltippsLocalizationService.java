...
import com.hackathon25.safenet.application.service.i18n.TranslationService;
import com.hackathon25.safenet.domain.enums.Language;
import com.hackathon25.safenet.domain.model.notfalltipps.*;
import com.hackathon25.safenet.domain.port.inbound.NotfalltippsLocalizationPort;
 ...
 *
 * <p>This service follows hexagonal architecture principles by:
 *
 * <ul>
 *   <li>Working exclusively with domain models (no DTOs)
 *   <li>Implementing inbound port for use case definition
 *   <li>Using outbound ports for dependencies
 *   <li>Keeping presentation concerns (DTO mapping) in adapters
 * </ul>
 *
...
public class NotfalltippsLocalizationService implements NotfalltippsLocalizationPort {
  ...
  private NotfalltippsRoot localizeContent(NotfalltippsRoot root, Language target) {
    ...
    List<NotfalltippsCategory> localizedCategories =
        root.getCategory() == null
            ? null
  ...
  private NotfalltippsCategory localizeCategory(
      NotfalltippsCategory category, Language source, Language target) {
    List<NotfalltippsTip> localizedTips =
        category.getTips() == null
            ? null
  ...
  private NotfalltippsTip localizeTip(NotfalltippsTip tip, Language source, Language target) {
    List<NotfalltippsArticle> localizedArticles =
        tip.getArticles() == null
            ? null
  ...
  private NotfalltippsArticle localizeArticle(
      NotfalltippsArticle article, Language source, Language target) {
    NotfalltippsImage localizedImage =
        article.getImage() == null ? null : localizeImage(article.getImage(), source, target);
  ...
  private NotfalltippsImage localizeImage(
      NotfalltippsImage image, Language source, Language target) {
