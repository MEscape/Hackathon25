...
export function useSafetyAlerts() {
  const { data: policeData, isLoading: policeLoading, isError: policeError } = useGetPoliceAlertsQuery();
  const { data: weatherData, isLoading: weatherLoading, isError: weatherError } = useGetWeatherAlertsEnglishQuery();
