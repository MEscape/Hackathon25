...
import { WarningsHeatmap } from '../components/WarningsHeatmap';
...
export default function MapScreen() {
  ...
  // Location data
  const { hasPermission, isTracking, isVisible, currentLocation } =
    useLocation();
  const { data: friendsLocations, isLoading: friendsLoading } =
    useGetFriendsLocationsQuery(undefined, { pollingInterval: 30000 });
  ...
  const { warnings, isLoading: warningsLoading } = useWarnings({
    center,
    radiusKm: 10,
    useMockData: false,
  });
  ...
  return (
    <Screen preset="fixed" style={themed($container)}>
      ...
      <MapStatusBar
        ...
        isLoadingWarnings={warningsLoading}
