...
interface HelpCentersOverlayProps {
  ...
  onHelpCenterPress?: (
    helpCenter: HelpCenter & { type: HelpCenterType }
  ) => void;
}

const HELP_CENTER_CONFIG: Record<
  HelpCenterType,
      ...
      centers.forEach((helpCenter: HelpCenterItem) => {
        ...
        markers.push(
          <TouchableOpacity
            ...
            onPress={() =>
              onHelpCenterPress?.({ ...helpCenter, id: uniqueKey, type })
            }
            activeOpacity={0.7}
          >
            <Icon icon={config.icon as any} size={16} color="#FFFFFF" />
          </TouchableOpacity>
        );
      });
    });
