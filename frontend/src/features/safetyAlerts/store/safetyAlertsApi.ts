...
import { PoliceAlert, PoliceAlertsResponseSchema } from '../schemas/safetyAlerts.schema';

export const safetyAlertsApi = apiSlice.injectEndpoints({
  overrideExisting: true,
  endpoints: builder => ({
    getPoliceAlerts: builder.query<PoliceAlert[], void>({
      query: () => '/api/nina/police/alerts',
      transformResponse: (response: unknown) => {
        const validated = validateApiResponse(PoliceAlertsResponseSchema, response);
