...
import { generateContextualResponse, generateFallbackResponse } from '../services/responseGenerator';
    ...
    try {
      ...
      if (onnxSession) {
        try {
          ...
          relevantTips = await modelService.runInference(query, emergencyTips, 3);
        } catch (error) {
          console.warn('ONNX inference failed, falling back to text processing:', error);
      ...
      const response = relevantTips.length > 0
      ...
      addMessage('assistant', translate('emergencyAssistant:responses.general'));
    } finally {
