        ...
        <View style={themed($card)}>
            <View style={themed($content)}>
                ...
                <View style={themed($avatarContainer)}>
                    <View style={[themed($avatar), { backgroundColor: getAvatarColor(displayName) }]}>
                        <Text style={themed($avatarText)}>{displayName.charAt(0).toUpperCase()}</Text>
                    </View>
                    <View style={[themed($statusIndicator), { backgroundColor: isOnline ? theme.colors.success : theme.colors.textDim }]} />
                </View>

                {/* Info Section */}
                <View style={themed($info)}>
                    <Text style={themed($name)} numberOfLines={1}>{displayName}</Text>
                    <Text style={themed($email)} numberOfLines={1}>{contact.email}</Text>

                    <View style={themed($statusRow)}>
                        <View style={[themed($statusDot), { backgroundColor: isOnline ? theme.colors.success : theme.colors.textDim }]} />
                        <Text style={themed($statusText)}>
                            {isOnline ? translate('emergencyContacts:status.online' as TxKeyPath) : (contact.lastSeen || translate('emergencyContacts:status.offline' as TxKeyPath))}
                        </Text>
                    </View>
                </View>
