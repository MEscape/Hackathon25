...
export const useProfile = () => {
  ...
  // Actions
  const handleSetMetadata = useCallback((newMetadata: UserMetadata) => {
  ...
  const handleSetEditing = useCallback((editing: boolean, field?: string) => {
  ...
  const handleUpdateBloodType = useCallback(async (bloodTypeValue: string) => {
    try {
      ...
      handleSetError(error?.data?.message || translate('profile:messages.updateError'));
    }
  }, [updateBloodType, handleSetError]);

  const handleUpdateJob = useCallback(async (jobValue: string) => {
    try {
  ...
  const handleUpdateAllergies = useCallback(async (allergiesValue: string) => {
    try {
  ...
  const handleUpdatePreExistingConditions = useCallback(async (conditionsValue: string) => {
    try {
  ...
  const handleUpdateMedication = useCallback(async (medicationValue: string) => {
    try {
  ...
  const handleUpdateVaccinationStatus = useCallback(async (vaccinationValue: string) => {
    try {
